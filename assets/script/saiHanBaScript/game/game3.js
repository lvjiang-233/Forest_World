cc.Class({
    ...
    createZombies(formula) {
        ...
        // 为僵尸设置一个计算公式
        zombies.getComponent("zombies3").getFormula(formula); // 子节点字符串名称需要修改
    ...
    createPeas(posY) {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            if (rand == i){
                console.log("yes");
                peas.getComponent("peas3").getAnswer(this.formula[1],true,posY);
            }
            else{
                ...
                peas.getComponent("peas3").getAnswer(temp, false, posY);
            }
